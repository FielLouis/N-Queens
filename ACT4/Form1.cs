...
namespace ACT4
{
    public partial class Form1 : Form
    {
        ...
        private void simulatedAnnealing()
        {
            ...
            {
                Point newMove = (Point)chosenMove;
                SixState newState = new SixState(currentState);
                newState.Y[newMove.X] = newMove.Y;

                int currentEnergy = getAttackingPairs(currentState);
                int newEnergy = getAttackingPairs(newState);

                if(acceptanceProbability(currentEnergy, newEnergy, temperature) > new Random().NextDouble())
                {
